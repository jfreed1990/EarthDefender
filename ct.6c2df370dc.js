...
(() => {
  ...
  var exportedSounds = [
    [{"name":"Shoot","variants":[{"uid":"WKpWkbGqHDD6D8","source":"./snd/WKpWkbGqHDD6D8.wav"}],"preload":false,"panning":{"refDistance":0,"rolloffFactor":0}},{"name":"Hit","variants":[{"uid":"Fw8LkwMk7D8j28","source":"./snd/Fw8LkwMk7D8j28.wav"}],"preload":false,"panning":{"refDistance":0.5,"rolloffFactor":1}},{"name":"BGM","variants":[{"uid":"k8d3ngDNb5WQ3Q","source":"./snd/k8d3ngDNb5WQ3Q.wav"}],"preload":true,"panning":{"refDistance":0.5,"rolloffFactor":1}}]
  ][0] ?? [];
  var soundMap = {};
  ...
  var resLib = {
    ...
    async loadGame() {
      ...
      const atlases = [
        ["./img/a0.{webp,png}.ec13ed332f.json"]
      ][0];
      const tiledImages = [
        {"Background":{"source":"./img/t0.87ed7fd666.{webp,png}","shape":{"type":"rect","top":0,"bottom":400,"left":0,"right":576},"anchor":{"x":0,"y":0}}}
      ][0];
      const bitmapFonts = [
        []
      ][0];
...
${ev.error?.stack ?? "(no stack available)"}`;
  ...
  
  {
    ...
    inputs.addAction('Shoot', [{"code":"keyboard.Space"},{"code":"pointer.Primary"}]);
inputs.addAction('MoveX', [{"code":"keyboard.KeyA","multiplier":-1},{"code":"keyboard.KeyD"},{"code":"keyboard.ArrowLeft","multiplier":-1},{"code":"keyboard.ArrowRight"}]);
...
templates.templates["PlayerShip"] = {
    name: "PlayerShip",
    depth: 0,
...
{
    const other = place.occupied(this, 'Hostile');
...
templates.list['PlayerShip'] = [];
        
templates.templates["Small_Asteroid"] = {
    name: "Small_Asteroid",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* template Small_Asteroid — core_OnStep (On frame start event) */
{
this.move();
...
/* template Small_Asteroid — place_collisionTemplate (Collision with a template event) */
{
    const other = place.meet(this, 'Bullet');
    if (templates.valid(other)) {
        other.kill = true;
this.kill = true;
sounds.play('Hit');
rooms.current.score += 50;
    }
}
    ...
    onCreate: function () {
        /* template Small_Asteroid — core_OnCreate (On create event) */
{
...
/* template Asteroid — place_collisionTemplate (Collision with a template event) */
{
    const other = place.meet(this, 'Bullet');
    if (templates.valid(other)) {
        other.kill = true;
this.kill = true;
templates.copy('Small_Asteroid', this.x, this.y);
templates.copy('Small_Asteroid', this.x, this.y);
sounds.play('Hit');
rooms.current.score += 150;
    }
}
...
this.timer1 = 3;
templates.copy('Torpedo', this.x, this.y + 32);
...
templates.list['EnemyShip'] = [];
        
templates.templates["Torpedo"] = {
    name: "Torpedo",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    behaviors: JSON.parse('[]'),
    onStep: function () {
        /* template Torpedo — core_OnStep (On frame start event) */
{
...
this.move();
    ...
    onCreate: function () {
        /* template Torpedo — core_OnCreate (On create event) */
{
this.speed = 480;
this.direction = 90;
}
...
templates.list['Torpedo'] = [];
        
templates.templates["ScoreText"] = {
    name: "ScoreText",
    depth: 1000,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    onDraw: function () {
        /* template ScoreText — core_OnDraw (On frame end event) */
{
...
templates.list['ScoreText'] = [];
        
templates.templates["LivesText"] = {
    name: "LivesText",
    depth: 1000,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
    ...
    onDraw: function () {
        /* template LivesText — core_OnDraw (On frame end event) */
{
...
rooms.templates['Main'] = {
    ...
    height: 400,
    behaviors: JSON.parse('[]'),
    objects: JSON.parse('[{"x":288,"y":320,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"PlayerShip"},{"x":32,"y":0,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"ScoreText"},{"x":448,"y":0,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"LivesText"}]'),
    bgs: JSON.parse('[{"texture":"Background","depth":-5,"exts":{"movementX":0,"movementY":0,"parallaxX":1,"parallaxY":1,"repeat":"repeat","scaleX":1,"scaleY":1,"shiftX":0,"shiftY":0}}]'),
    tiles: JSON.parse('[]'),
...
templates.copy(random.dice('Asteroid', 'Small_Asteroid'), random(camera.width), -100);
